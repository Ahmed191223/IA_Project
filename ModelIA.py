...
def predire_match(team1, team2):
    """
    Prédit le résultat, la possession et le score d’un match entre deux équipes.
    Retourne un dictionnaire contenant toutes les informations.
    """
    results = {
        "teams": (team1, team2),
        "error": None,
    ...
    if not features:
        results["error"] = f"Aucune donnée disponible pour le match {team1} vs {team2}"
        return results

    results["resume"] = resume
    results["confrontations"] = n_matchs
    ...
    results["possession"] = {
        team1.title(): round(p1_norm, 2),
        team2.title(): round(p2_norm, 2)
    }
    ...
    # Résultat final
    if result == 1:
        results["prediction"] = f"{team1.title()} a plus de chances de remporter le match."
    else:
        results["prediction"] = f"{team2.title()} semble favori ou un match nul est probable."
